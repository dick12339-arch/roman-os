...
export async function onRequestGet(context){
  ...
  try{
    if(!env.DB) return new Response(JSON.stringify({error:"D1 not bound - 請到 Pages Settings 綁定 DB"}), {status:500, headers:{"Content-Type":"application/json","Access-Control-Allow-Origin":"*"}});
...
export async function onRequestPost(context){
  ...
  try{
    ...
    if(!raw) return new Response(JSON.stringify({error:"raw empty - 沒有資料"}), {status:400, headers:{"Content-Type":"application/json","Access-Control-Allow-Origin":"*"}});
    if(isSchedule){
